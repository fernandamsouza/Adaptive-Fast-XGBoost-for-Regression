...
import time
import adaptive_xgboost as axgboost
...
from river.drift import DDM, ADWIN, KSWIN
...
def PWPAE(X_train, y_train, X_test, y_test):
    ...
    hat1 = ensemble.AdaptiveRandomForestClassifier(n_models=3, drift_detector = ADWIN()) # ARF-ADWIN
    hat2 = axgboost.AdaptiveXGBoostClassifier()
    hat3 = ensemble.AdaptiveRandomForestClassifier(n_models=3, drift_detector = DDM()) # ARF-DDM
    hat4 = ensemble.SRPClassifier(n_models=3, drift_detector = DDM()) # SRP-DDM

    # The four base learners learn the training set
    for xi1, yi1 in stream.iter_pandas(X_train, y_train):
        hat1.learn_one(xi1,yi1)
        hat2.partial_fit(np.array((list(xi1.values()))),yi1)
    ...
    for xi, yi in stream.iter_pandas(X_test, y_test):
        ...
        y_pred2= hat2.predict(xi) 
        y_prob2= hat2.predict_proba(xi)
        hat2.partial_fit(xi,yi)
...
df = pd.read_csv("./data/Keystroke.csv")
...
# # Use the Adaptive Random Forest (ARF) model with KSWIN drift detector
# name1 = "ARF-ADWIN model"
# model1 = ensemble.AdaptiveRandomForestClassifier(n_models = 3) # Define the model

...
class Adaptive4(BaseSKMObject, ClassifierMixin):
    ...
    def _partial_fit(self, X, y):
        ...
        # print("---- DEBBUGER ----")
        # print("--- _X_buffer: %s ---" % self._X_buffer)
        # print("--- _y_buffer: %s ---" % self._y_buffer)
    ...
    def _train_booster(self, X: np.ndarray, y: np.ndarray, last_model_idx: int):
        ...
        if self._booster:
            ...
            booster = xgb.train(params=self._boosting_params,
                                ...
                                num_boost_round=1,
                                xgb_model='model_2.model')
            booster.save_model('model_2.model')
        else:
            booster = xgb.train(params=self._boosting_params,
                                dtrain=d_mini_batch_train,
                                num_boost_round=1,
    ...
    def predict(self, X):
        ...
        if self._booster:
            ...
            # print("--- predicted int: %s ---" % np.array(predicted > 0.5).astype(int))
            # print("--- window size: %s ---" % self.window_size)
